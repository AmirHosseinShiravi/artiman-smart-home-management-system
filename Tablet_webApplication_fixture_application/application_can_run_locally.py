...
def install_lets_encrypt_certs():
    ...
    sleep(1)
    try:
        for file_path in glob("resources/letsencrypt-certs/pem_cert_files/*.pem"):
            file_path = Path(file_path)
            file_name = file_path.parts[-1]
            cert_name = file_name.replace("-", " ").upper()
            console.print(f"[bold magenta]Using let's encrypt {cert_name} certificate.")
            output = command_process([os.path.join(current_path, Path("resources/openssl/openssl.exe")), "x509", "-inform", "PEM", "-subject_hash_old", "-in", Path(f"resources/letsencrypt-certs/pem_cert_files/{file_name}")])
            ...
            copy(Path(f"resources/letsencrypt-certs/pem_cert_files/{file_name}"), Path(f"resources/letsencrypt-certs/generated_certs/{cert_subject_hash}.0"))
            source_path = os.path.join(current_path, Path(f"resources/letsencrypt-certs/generated_certs/{cert_subject_hash}.0"))
...
def install_com_google_android_webview_application():
    try:
        ...
        webview_apk_source = str(Path("resources/ChromiumWebview/webview.apk"))
        webview_lib_source_path = str(Path("resources/ChromiumWebview/lib"))
...
def replace_framework_res_apk_file():
    try:
        ...
        os.makedirs(Path(f"resources/original_framework_res_files/{adb._target}"), exist_ok=True)
        destination_path = str(Path(f"resources/original_framework_res_files/{adb._target}/"))
        adb.shell_command(f"pull /system/framework/framework-res.apk {destination_path}")
        source_path = str(Path("resources/modified_framework_res/framework-res.apk"))
...
def restore_original_framework_res_file():
    try:
        ...
        source_path = str(Path(f"resources/original_framework_res_files/{adb._target}/framework-res.apk"))
        adb.shell_command(f"push {source_path} /system/framework/framework-res.apk")
    except Exception as e:
