...
def get_first_timestamp(base_time: Union[None, int, float, timedelta, dt_time, datetime]) -> float:
    ...
    __local_timezone = pytz.timezone("Asia/Tehran")
    now = get_now(tz=__local_timezone)
    new_base: datetime
    ...
    if base_time is None:
        ...
        new_base = now.replace(hour=base_time.hour, minute=base_time.minute,
                               second=base_time.second, microsecond=base_time.microsecond)
        if new_base < now:
            new_base = new_base.replace(day=new_base.day + 1)
    else:
        assert isinstance(base_time, datetime)
        new_base = instance(base_time, tz=pytz.utc).astimezone(__local_timezone)
