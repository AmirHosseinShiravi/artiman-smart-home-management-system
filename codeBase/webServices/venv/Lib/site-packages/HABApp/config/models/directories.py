...
class DirectoriesConfig(BaseModel):
    ...
    rule_config: Optional[Path] = Field(Path('rule_config'), description='Folder where rule config files can be placed')
    ...
    def create_folders(self):
        ...
        if not self.logging.is_dir():
            self.logging.mkdir()
        if not self.rule_config.is_dir():
            self.rule_config.mkdir()
        ...
        if not self.rule_config.is_dir():
            log.info(f'rule_configs disabled! Folder {self.rule_config} does not exist!')
