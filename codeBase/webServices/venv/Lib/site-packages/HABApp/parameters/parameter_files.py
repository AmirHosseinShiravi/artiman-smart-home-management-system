...
rul_config_log = logging.getLogger('HABApp.RuleParameters')

LOCKForRuleConfig = threading.Lock()
Rule_Config_PREFIX = 'rule_config/'


async def load_rul_config_file(name: str, path: Path):
    # with LOCKForRuleConfig:  # serialize to get proper error messages
    #     with path.open(mode='r', encoding='utf-8') as file:
    #         data = HABApp.core.const.yml.load(file)
    #     if data is None:
    #         data = {}
    #     set_parameter_file(path.stem, data)
    # print(HABApp.core.main_app)
    # await HABApp.core.main_app.rule_manager.watcher.trigger_all()
    rul_config_log.debug(f'Loaded rule config from {name}!')


async def unload_rul_config_file(name: str, path: Path):
    with LOCKForRuleConfig:  # serialize to get proper error messages
        remove_parameter_file(path.stem)

    rul_config_log.debug(f'Removed rule config from {path.name}!')


class HABAppRuleConfigFile(HABAppFile):
    LOGGER = log
    LOAD_FUNC = load_rul_config_file
    UNLOAD_FUNC = unload_rul_config_file


async def setup_rule_config_files() -> bool:
    path = HABApp.CONFIG.directories.rule_config
    if path is None:
        return False

    folder = add_habapp_folder(Rule_Config_PREFIX, path, 101)
    folder.add_file_type(HABAppRuleConfigFile)
    watcher = folder.add_watch('.json')
    await watcher.trigger_all()
    return True
