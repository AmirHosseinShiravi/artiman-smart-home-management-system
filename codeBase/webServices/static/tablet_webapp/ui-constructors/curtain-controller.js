...
class CurtainAnimation {
    constructor({
        ...
        // reduced animation frame rate version
        this.targetFPS = 60; // Adjust this value based on performance
    ...
    animateCurtains(targetProgress) {
        ...
        const animate = (currentTime) => {
            ...
            if (animationProgress < 1 && this.isAnimating) {
                ...
                // reduced animation frame rate version
                if (currentTime - lastTime < this.frameInterval) return;
...
class CurtainController {
            ...
            <div style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;">
                <div class="" style="display: flex;height: 250px;align-items: flex-end;">
                    <div class="text-section d-flex flex-column align-items-start">
                        <div class="big-text animation-text-container overflow-text"><span id="card-name">${curtain.button_name}</span></div>
                        <div class="little-text"><span id="card-status">${curtain.off_text}</span></div>
                    </div>
                </div>
                <div class="">
                    <div class="icon-section d-flex align-items-end" style="${curtain.background_image_style};background-image: url('${curtain.background_image}')">
                    </div>
                </div>
            </div>
        `;
    ...
    createCurtainCardForHomePage(curtain) {
    ...
    updateCurtainCard(curtain, isOpen) {
        const cards = document.querySelectorAll(`.custom-card[data-ui-element-uuid="${curtain.uuid}"]`);
        if (cards) {
            cards.forEach(card=>{
