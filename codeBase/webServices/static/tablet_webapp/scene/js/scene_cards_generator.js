...
function generateRuleCards() {
    ...
    set_cards_texts_color();

    const number_of_scene_rules = linkage_rules.filter(rule=> rule.type === "scene").length;
    const number_of_automation_rules = linkage_rules.filter(rule=> rule.type === "automation").length;
    if(number_of_scene_rules === 0){
        tapToRunSection.appendChild(generate_empty_scene_content())
    }
    if(number_of_automation_rules === 0){
        automationSection.appendChild(generate_empty_scene_content())
    }
        ...
        <div class="row scene-rule-card" data-scene-${rule.type}-rule-id="${rule.index}" style="background-color: ${rule.style.color};">
            <div class="col-9 scene-rule-card-detail-section">
                <div class="icon-section">
                    ${rule.style.icon ? `<i class="${rule.style.icon}"></i>` : '<i class="fa-solid fa-wand-magic-sparkles"></i>'}
                </div>
            ...
            <div class="col-3 d-flex justify-content-end scene-rule-card-action-section">
                <div class="d-flex flex-column">
                    ...
                    <div class="rule-status-switch-container mt-auto">
...
function set_cards_texts_color() {
    linkage_rules.forEach(rule=> {
        const card_color = rule.style.color;
...
function add_home_page_enabled_tap_to_run_rules_to_home_page(){
    const home_page_scene_container = document.querySelector('.main-page.home-page .scene-container');
    ...
    if(all_tap_to_run_rules_with_add_to_home_flag){
        all_tap_to_run_rules_with_add_to_home_flag.forEach(rule => {
            ...
            tap_to_run_card_element.innerHTML = `<div class="row home-scene-rule-card" style="background-color: ${rule.style.color};">
    ...
    home_page_scene_container.appendChild(row_element);
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.scene-rule-card .scene-rule-card-detail-section').forEach(card => {
...
function removeAllEventListeners() {
    ...
    document.querySelectorAll('.main-page.home-page .top-container .scene-container button').forEach(button => {
